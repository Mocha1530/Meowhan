...
local function connectDestroyEvent()
    print("Starting connectDestroyEvent")
    local uiScreenGui
    local attempts = 0
    ...
    while not uiScreenGui and attempts < 10 do
        ...
        if not uiScreenGui then
            attempts += 1
            warn("UI not found, attempt: " .. attempts)
            task.wait(0.5)
        end
    end

    if uiScreenGui then
        warn("UI found: " .. uiScreenGui:GetFullName())
        
        -- Store the original parent for reference
        local originalParent = uiScreenGui.Parent
        warn("UI parent: " .. originalParent:GetFullName())
        
        -- Connect to Destroying event
        local destroyConnection = uiScreenGui.Destroying:Connect(function()
            warn("Destroying event fired!")
            ...
            if scalingLoop then
                ...
                warn("scalingloop destroyed")
            end
            
            if restoreOriginalProperties then
                restoreOriginalProperties()
                warn("restore properties called")
            end
        end)
        
        -- Connect to AncestryChanged event
        local ancestryConnection
        ancestryConnection = uiScreenGui.AncestryChanged:Connect(function()
            warn("AncestryChanged event fired! New parent: " .. tostring(uiScreenGui.Parent))
            if not uiScreenGui:IsDescendantOf(game) then
                warn("UI is no longer a descendant of game")
                for key, _ in pairs(Running) do
                    Running[key] = false
                end
        ...
        -- Also track the destroy connection
        if UILib and UILib.TrackProcess then
            UILib:TrackProcess("connections", destroyConnection, "destroyConnection")
            UILib:TrackProcess("connections", ancestryConnection, "ancestryConnection")
        end
        
        -- Add a manual check as a fallback
        task.spawn(function()
            while task.wait(1) do
                if not uiScreenGui or not uiScreenGui:IsDescendantOf(game) then
                    warn("Manual check detected UI destruction")
                    for key, _ in pairs(Running) do
                        Running[key] = false
                    end
        ...
        task.spawn(function()
            while task.wait(1) do
                local currentUI = CoreGui:FindFirstChild("MeowhanUI") or PlayerGui:FindFirstChild("MeowhanUI")
                if not currentUI then
                    warn("Fallback check detected UI destruction")
                    for key, _ in pairs(Running) do
                        Running[key] = false
                    end
                    ...
                    if scalingLoop then
                        scalingLoop:Disconnect()
                        warn("scalingloop fallback")
                    end
                    
                    if restoreOriginalProperties then
                        restoreOriginalProperties()
                        warn("restore fallback")
                    end
                    break
                end
            end
